...
AppState appReducers(AppState state, dynamic action) {
  ...
  } else if (action is SetSUPHomePageEditingMode) {
    return setSUPHomePageEditingMode(state, action);
  } else if (action is SetCourseToView) {
    return setCourseToView(state, action);
  } else if (action is SetSUPsubjectsPageEditingMode) {
    return setSUPsubjectsPageEditingMode(state, action);
  } else if (action is SetCardEditingMode){
    return setCardEditingMode(state, action);
  }
  return state;
}
...
AppState updateChoosenCoursesCards(
    AppState state, UpdateChoosenCoursesCards action) {
  return state.cloneAndUpdateValue(
      'choosenCoursesCards', action.choosenCoursesCards);
}
...
AppState setSUPHomePageEditingMode(
    AppState state, SetSUPHomePageEditingMode action) {
  Logger()
      .i('setting SUP home page editing mode to ' + action.state.toString());
  return state.cloneAndUpdateValue('SUPhomePageEditingMode', action.state);
}

AppState setCourseToView(
    AppState state, SetCourseToView action) {
  Logger()
      .i('setting SUP course to view to ' + action.course.toString());
  return state.cloneAndUpdateValue('CourseToView', action.course);
}

AppState setSUPsubjectsPageEditingMode(
    AppState state, SetSUPsubjectsPageEditingMode action) {
  Logger()
      .i('setting SUP subjects page editing mode to ' + action.state.toString());
  return state.cloneAndUpdateValue('SUPsubjectsPageEditingMode', action.state);
}

AppState setCardEditingMode(
    AppState state, SetCardEditingMode action) {
  Logger()
      .i('setting course card editing mode to ' + action.state.toString());
  return state.cloneAndUpdateValue('CardEditingMode', action.state);
}
